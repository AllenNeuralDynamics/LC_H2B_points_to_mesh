...
Reproduces the paper figures from the meshes this capsule generates: kNN
minimum-projection heatmaps, 3D mesh/brain-outline renderings, per-sample counts,
coronal slices with PC2 histograms, and a raw-image max-projection overlay. Plus a
couple of lightweight exploration helpers used by the notebook.
...
def plot_all_percentile_meshes(percentile_meshes, brain_mesh=None, save_path=None):
    ...
    fig = go.Figure()
    # Draw the largest (90th) first and most transparent, the smallest (10th) last and
    # most opaque, so the inner meshes stay visible instead of being hidden behind the
    # outer shells. (Iterating mesh_keys directly, not reversed, is the fix for the
    # earlier double-reverse that drew the largest mesh opaque and on top.)
    mesh_keys = sorted(percentile_meshes.keys(), reverse=True)  # 90..10
    for i, mesh_key in enumerate(mesh_keys):
